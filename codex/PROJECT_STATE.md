...
## Projektstatus
Status: Phase-1-Kern abgeschlossen; Phase 2A, 2B, 2C, 2D, 2E, 3A, 3B, 4A, 4B und 4C erweitern den Core jetzt um regelbasierte Scene-/Shot-Planung, Mehrfach-Takes, technischen Quality-Guard, validierte Take-Selektion, kreative Varianten pro Szene, leichte inhaltliche Auswahlheuristik, optionale Storyboard-/Keyframe-Vorsteuerung, produktive First-Frame-Keyframe-Konditionierung im bestehenden `ti2vid`-Pfad, eine minimale produktive Worker-/n8n-Bridge ueber FastAPI, einen polling-faehigen Async-Submit-Pfad, n8n-freundliche Polling-Hinweise und begrenzte Retries; Phase 5A erweitert den Planner jetzt um eine Director-/Brain-Schicht mit `director_output`, `style_lock`, staerkeren Szenenintents, verbessertem Prompt-Bau und ehrlichem lokalem LLM-Fallback; Phase 5B bindet diesen Director-Layer jetzt produktiv an ein echtes lokales Qwen3.6-35B-A3B-Serving ueber `llama.cpp` + GGUF an; Phase A des aktuellen Output-Quality-Fokus fuehrt einen kleinen Scene World Contract und PromptBuilder v2 fuer haertere Szene-/Variation-Prompts ein; Phase B1 macht Storyboard-/Keyframe-Prompts jetzt scene-specific und contract-aware; Phase B2 fuehrt einen leichten Keyframe Visual Risk Review fuer Storyboard-Kandidaten ein; Phase C fuehrt Take Visual Review, Review-Frame-Extraction und Postability Score mit optionalem Qwen3-VL Provider ein; Phase D fuehrt ein Final Quality Verdict fuer `final.mp4` ein; Phase G1 fuehrt declarative `pipeline_defs`, generische Checkpoints und lokale dateibasierte Approval-Gates als kontrollierbare Produktionsmaschinen-Schicht ein; Phase G1.1 macht diese Checkpoints in der CLI inspizierbar und schreibt lokale Approval-/Reject-Dateien kontrolliert; Phase G2 fuehrt jetzt eine OpenMontage-inspirierte Skill-Schicht, skill-aware Pipeline Modes, Creative Role Contracts, Prompt-/Skill-Trace und einen vorbereiteten Decision Log ein; der Restore-/Startup-Pfad nach Repo-Update und Pod-Neustart ist jetzt erneut real geprueft, minimal gehaertet und um eine robuste lokale Director-Env-/Serve-Konfiguration erweitert
...
- V0.2-Snapshots liegen unter `/workspace/cockpit_snapshots_2026-05-09_v02/`; Presprint-/Final-Archive liegen unter `/workspace/session_archive_2026-05-09_cockpit_presprint/` und `/workspace/session_archive_2026-05-09_cockpit_final/`.
- 2026-05-09 Stage 09 Image Jobs Pass: Active Workspace `PROMPTS / IMAGE JOBS` zeigt einzelne Image-Job-Bloecke fuer scene_01 bis scene_03 mit Preview-Zone, Status und Expand/Collapse.
- Stage-09-Bedienung ist lokal: j/k waehlt Image Jobs, Enter/Space klappt den selektierten Job auf/zu; Fixture-Demo zeigt ready, generating mit Demo-Progress/%/elapsed/backend und in queue.
- Keine globale Layoutaenderung, kein Header-/Sidebar-/Bottom-Panel-Umbau, keine Pipeline-Integration, kein Render, keine API-/n8n-Arbeit und keine neuen Runs.
- 2026-05-09 Textual Cockpit Stage Router V0.1: Pipeline Map V1 00-15 ist als Operator-Sicht eingebaut und ersetzt im Cockpit-UI die alte interne 01-12-Liste.
- Active Workspace routet nach selected Stage: 00 Command Center, 01 Pipeline wählen, 02 Mode & Style und 03 Skills laden sind initial gestaltet; 04-15 zeigen read-only Platzhalter, Stage 09 erhaelt das bestehende Prompts/Image-Jobs-Panel.
- Auswahl ist per Pfeil hoch/runter und j/k stabil; selected Stage ist sichtbar markiert, der aktuelle technische Stage bleibt separat hervorgehoben. Klick-Auswahl ist als Textual-Handler vorbereitet.
- Keine Live-Pipeline-Integration, keine echte CLI-Eingabe, kein Job-Submit, kein Render, keine API-/n8n-Arbeit und keine Fake-Livewerte.
...
- `PipelineDefinition` und `PipelineStepDefinition` koennen jetzt `required_skills` deklarieren; Pipeline-Definitionen koennen zusaetzlich `stage_roles` tragen.
- Neue Pipeline-Definition `agent_core/pipeline_defs/clean_shortform_v1.json` beschreibt einen skill-aware Social-Shortform-Flow mit Creative Strategy, Beat Plan, Visual Direction, Model Prompt Compile, Review, Selection, Assembly und Final Gate.
- Neue Creative Role Contracts in `agent_core/schemas.py`: `CreativeStrategy`, `BeatPlan`, `VisualDirection`, `ModelPromptPlan`, `ReviewPlan`, plus `DecisionLog`/`DecisionLogEntry`.
- `prompt_audit.json` und `model_prompts.json` enthalten jetzt `pipeline_id`, `required_skills`, `loaded_skills`, `missing_skills`, `stage_roles`, Backend-Prompt-Policy-Notizen und LTX-positive/negative Trace-Felder.
- `decision_log.json` wird im Run-Workspace vorbereitet/geschrieben und protokolliert initial `selected_pipeline`, `selected_mode`, `selected_style`, `selected_skill_set`, `selected_motif_family`, `selected_shot_recipe`, `backend_prompt_policy` sowie Future-Work-Eintraege fuer Take Selection und Final Quality Decision.
- Morning Reset ist weniger starr: `motif_families` und `motif_family_guidance` beschreiben flexible Motivfamilien; die bisherigen Scene-Arc-Rezepte bleiben als kompatible empfohlene Bausteine erhalten.
- CLI-Safety-Flags sind ergaenzt: `--pipeline-dry-run` und `--approval-gates-enabled` setzen nur Job-Metadata und starten selbst keine Modelle/Render.
- G2 hat keine Render gestartet, keine Modelle geladen, keine Downloads, keine Runtime-/Docker-/init.sh-/Backend-/n8n-/API-/GUI-Aenderungen gemacht.
- 2026-05-02 Final Mega Task G3/G4/G5 Architektur-Schicht ist defensiv umgesetzt.
- G3: Stage Role Contracts sind produktiv tracebar ueber `agent_core/creative_system/contracts.py`, `stage_contracts.json`, `plan.metadata.stage_contracts`, `prompt_audit.json` und `model_prompts.json`.
- G3-Vertraege decken `CreativeStrategy`, `BeatPlan`, `VisualDirection`, `ModelPromptPlan` und `ReviewPlan` ab; bestehende Runtime bleibt rueckwaertskompatibel.
- G4: `--stop-after {scene_plan,model_prompts,storyboard}` setzt `job.metadata.stop_after`; Core stoppt sicher vor spaeteren Backend-/Render-Schritten und schreibt `stopped_after`, `produced_artifacts`, `next_action`, Checkpoints und Result-Metadata.
- G4: Resume bleibt bewusst kein Executor. `agent_core/resume_contract.py` erkennt Approval-/Reject-Dateien, wiederverwendbare Artefakte und dokumentiert Idempotenzregeln.
- G5: Metadata-basierte Creative-Quality-Review-Warnungen fuer boring scene, weak hook, unclear action, generic stock feel, physical incoherence, bad composition, poor platform fit, no visual change und voice/visual mismatch sind vorbereitet.
- G5: `evaluate_final_quality_verdict()` akzeptiert und persistiert `creative_quality_warnings` und `platform_fit_warnings`; echte VLM-Inferenz wird dabei nicht gefaked.
- G5: `decision_log.json` wurde um `approval_gate_status`, `stop_after` und `quality_decision`-Contract erweitert.
- Sicherer Smoke ohne Render: `/workspace/agent_runs/g5-final-stop-after-model-prompts-smoke` erzeugt `state.json`, `checkpoints.json`, `stage_contracts.json`, `decision_log.json`, `prompt_audit.json`, `model_prompts.json`, aber kein `final.mp4`.
- 2026-05-02 Phase G1.1 CLI Checkpoint Inspect + Approval/Reject UX ist umgesetzt.
- `scripts/agent_core_cli.py --inspect-run <job>` zeigt jetzt zusaetzlich Checkpoints aus `checkpoints.json` oder fallback aus `state.json.checkpoints`.
- Neuer Alias/Modus: `--inspect-checkpoints <job>` zeigt Pipeline, current/blocked checkpoint, Statusliste, Stage, Blocking, Approval Required, Reason, Issues, Warnings, Related Artifacts und Approval-Dateipfade.
- Neue lokale Gate-Befehle: `--approve-checkpoint <job> <checkpoint_id> --approved-by ... --approval-note ...` und `--reject-checkpoint <job> <checkpoint_id> --rejected-by ... --approval-note ...`.
- Approval-/Reject-Dateien werden nur im Run-Ordner unter `approvals/<checkpoint_id>.json` geschrieben; existierende Dateien werden nur mit `--force-approval` ueberschrieben.
- Live-/Append-Dashboard zeigt bei vorhandenen State-Feldern `current_checkpoint_id` und `blocked_by_checkpoint_id` einen `CHECKPOINT`-Block mit Status, Approval-Pflicht und Next-Action-Hinweis.
- Resume ist in G1.1 nur UX-vorbereitet: CLI zeigt fehlende Approval-Datei und Befehl, aber ein echter Resume-Executor bleibt Future Work.
- Tests fuer G1.1 sind gruen: `python -m unittest tests/test_cli_checkpoints.py -v` -> 6 Tests OK; zusammen mit G1 `python -m unittest tests/test_pipeline_g1.py tests/test_cli_checkpoints.py -v` -> 12 Tests OK.
- 2026-05-02 Phase G1 Pipeline Definitions / Checkpoints / Approval Gates ist umgesetzt.
- Neue Pipeline-Definition: `agent_core/pipeline_defs/simple_video_v1.json` mit Steps `validate_job`, `create_plan`, `approve_plan`, `create_prompts`, `approve_prompts`, `generate_voice_optional`, `render_video`, `assemble`, `final_quality_gate`.
- Neue Verträge in `agent_core/schemas.py`: `PipelineDefinition`, `PipelineStepDefinition`, `PipelineRetryPolicy`, `PipelineApprovalPolicy`, `CheckpointRecord`, `CheckpointStatus` und `ApprovalMode`.
- Neuer Loader/Gate-Code in `agent_core/pipeline.py`: Pipeline-Definitionen werden aus JSON geladen, Checkpoints aus Step-Definitionen gebaut, Approval-Dateien liegen unter `/workspace/agent_runs/<job_id>/approvals/<checkpoint_id>.json`.
- `JobState` speichert jetzt `pipeline_id`, `checkpoints`, `current_checkpoint_id` und `blocked_by_checkpoint_id`; `StateStore` persistiert zusaetzlich `checkpoints.json`.
- `VideoAgent.run_job()` setzt Checkpoints fuer Validierung, Plan, Plan-Approval, Prompt-Erzeugung, Prompt-Approval, optional Voice, Render, Assembly und Final Quality Gate.
- Approval-Gates blockieren nur bei `job.metadata.approval_gates_enabled=true`; ohne diese Metadata werden sie fuer Rueckwaertskompatibilitaet auto-passed.
- `job.metadata.pipeline_dry_run=true` stoppt nach Plan/Prompt-Checkpoints vor Voice/Storyboard/Video/Modell-Backends und schreibt trotzdem `prompt_audit.json`, `model_prompts.json`, `checkpoints.json`, `state.json` und `result.json`.
- Tests fuer G1 sind gruen: `python -m unittest tests/test_pipeline_g1.py -v` -> 6 Tests OK; zusaetzlich `python -m compileall -q agent_core tests/test_pipeline_g1.py` und `python -m unittest tests/test_planner_rules.py tests/test_creative_system.py -v` -> 19 Tests OK.
- Es wurde kein Render gestartet, keine Modelle geladen, keine Runtime-/Docker-/init.sh-/Backend-/n8n-/GUI-Aenderung vorgenommen.
