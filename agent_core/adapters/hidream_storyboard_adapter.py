...
class HiDreamStoryboardAdapter(StoryboardAdapter):
    name = "hidream_storyboard"
    ...
    def capabilities(self) -> BackendCapabilities:
        notes = ["uses local FastAPI /hidream endpoints for storyboard keyframes"]
        try:
            payload = http_json(f"{self.base_url}/DW/hidream_ready")
    ...
    def generate_storyboard(self, job: JobInput, plan: ProductionPlan, workspace: Path) -> ExecutionResult:
        ...
        submit = http_json(f"{self.base_url}/hidream/jobs", method="POST", payload=payload, timeout=60)
        backend_job_id = str(submit["job_id"])
        status_url = f"{self.base_url}/hidream/jobs/{backend_job_id}"
        ...
        while time.monotonic() < deadline:
            ...
            raise RuntimeError(f"HiDream storyboard job {backend_job_id} timed out after {self.max_wait_sec}s")
        ...
        if str(latest_status.get("state")) != "succeeded":
            return ExecutionResult(
                ...
                error=str(latest_status.get("error") or "hidream storyboard failed"),
    ...
    def _resolve_effective_prompt(self, plan: ProductionPlan, storyboard_step) -> tuple[str, str]:
        ...
        if isinstance(policy, dict) and str(policy.get("hidream") or "") == "positive_only":
